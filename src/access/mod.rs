//! The OPA gate. Enforcement lives in the typed per-op hooks, which see the full parsed
//! request — including the riders (canned ACL, `x-amz-grant-*`, inline `x-amz-tagging`,
//! governance bypass) a path-only check would miss. `check` is the deny-by-default
//! backstop: it refuses anonymous callers and every op [`optable`] does not mark
//! [`Coverage::Enforced`], because the default hooks fail OPEN. Every refusal is audited;
//! an allowed record is held for the forward leg ([`crate::audit::PendingAudit`]), a
...
///
/// A macro rather than a trait: the four input types share these fields by coincidence of
/// the S3 API, and naming every field explicitly makes "the hook forgot one" a compile
/// error at the call site rather than a silent `None`.
...
///
/// Built once per typed hook from `&mut S3Request<T>`: the s3s generics stop at the hook
/// boundary, so everything the enforce path may need from the request goes here rather
/// than making every helper generic over the op input type.
struct ReqCtx<'a> {
    ...
    operation: Arc<str>,
    meta: RequestMeta,
    /// The live extensions of the request being enforced — where the authz proof, the
    /// response obligations and the pending-audit handle are stashed.
...
impl<'a> ReqCtx<'a> {
    ...
    ///
    /// One place rather than four, so no write hook can build an input that forgets the
    /// ACL.
    fn write_input(
        &self,
        action: Action,
...
impl GatewayAccess {
    ...
    ///
    /// The single funnel: every `Pdp::decide` on the request path goes through here, so
    /// the golden-capture tap sees 100% of emitted inputs without instrumenting a hook. A
    ...
    ///
    /// Stashed next to the authorization proof, and for the same reason: the forward path
    /// is the single place that knows what happened next. If nothing ever settles it,
    /// `PendingAudit`'s `Drop` emits it unenriched — the record is deferred, never
    /// conditional.
    fn audit_pending(
        &self,
        cx: &mut ReqCtx<'_>,
    ...
    ///
    /// Rate-limited inside the sink (`AuditSink::emit_gate_denial`): this is the one audit
    /// stream an unauthenticated caller controls the rate of, and letting a scanner fill
    /// the queue would make the gateway drop *decision* records.
    ...
    /// Ask the PDP about one prepared input, then act on the answer: hold the record and
    /// mint the proof on allow, emit the record and refuse on deny. Every single-decision
    ...
    async fn enforce(&self, cx: &mut ReqCtx<'_>, input: OpaInput) -> S3Result<()> {
        ...
        // fail-open shape this project exists to avoid. Gated on `allow` so a request the
        // policy refused outright keeps the policy's own reason on the record.
        if decision.allow
    ...
    ///
    /// It audits **and** answers `AccessDenied`. Returning `Err` directly before the
    /// enforce call would return ahead of every audit site, leaving no evidence and
    /// reporting an authorization refusal as a 400 client formatting mistake.
    ///
    /// The `deny (gateway):` prefix says which layer refused: rego reasons all read
    /// `deny: …`/`allow: …`, so an unprefixed one would read as a policy verdict for a
    /// question no policy was asked.
    ...
    /// across several sub-decisions (a copy's two halves; a write plus the
    /// `write_object_tags` its inline `x-amz-tagging` requires). Prefixing those would
    ...
    /// revocation channel, and a long-lived copy would keep enforcing a reservation the
    /// control plane has since changed. Tag writes are not a hot path.
    ...
    /// may authorize:
    ///
    /// 1. **`x-amz-bypass-governance-retention`.** No verb in the grant vocabulary
    ///    expresses "may defeat WORM", so no grant could authorize it. The attempt is
    ///    still recorded (`input.bypass_governance`) — a regulated deployment has to be
    ///    able to ask who tried.
    /// 2. **Any conferring ACL**, public *or* named: it grants access the control plane
    ///    cannot show or revoke. Both come back as [`headers::AclDisposition::refusal`],
    ///    so a tier added to that enum later is screened without this line changing.
    ...
    ///
    /// An inline `x-amz-tagging` is a `PutObjectTagging` wearing a `PutObject`'s clothes,
    /// and must go through `write_object_tags` because a tag can satisfy an ABAC
    /// condition — which is also why that verb is not merged into `write_objects`.
    ///
    /// `Some(reason)` ⇒ refuse. Returns rather than denying inline so the caller records
    /// **one** audit entry against the primary input: a request produces one record,
    /// whatever it decomposed into.
    ...
    /// as their own verbs, then decide the write itself. A request with no ACL, no inline
    /// tags and no bypass header costs exactly one decision.
    async fn enforce_object_write(
        ...
        // return the audit path never saw: a refusal with no record is not a refusal
        // anyone can review.
    ...
    /// prefixes cannot narrow the decision — which is why these verbs are separate from
    /// the object ones rather than a keyless fall-through of them.
    ...
    /// The account-scoped enumeration path (`ListBuckets`), the one decision whose verdict
    /// is applied to a **response** rather than answered with a 403 — see
    /// [`GatewayAccess::list_buckets`] for why. Three outcomes, each writing exactly one
    /// audit record:
    ///
    ...
    ///    recorded as a gateway-side deny naming the obligation. Classified **before**
    ///    the audit call, or a refused `ListBuckets` would leave no record at all;
    ...
    async fn enforce_bucket_listing(&self, cx: &mut ReqCtx<'_>) -> BucketVisibility {
        ...
        // on `input.bucket != ""`, or a `"bucket": "*"` grant matches the empty string and
        // the per-bucket denylist is keyed on a bucket nobody named.
        // `the_wildcard_grant_does_not_match_the_account_scope` in the corpus is the guard.
        //
        // `Action::Read` in the ACCOUNT shape — the same verb `HeadBucket` uses in the
        // bucket shape, told apart only by the empty bucket, which is why every rego rule
        // reading it carries a shape gate.
    ...
    /// Copy-style path (CopyObject, UploadPartCopy): authorize BOTH the source read and
    /// the dest write. One request-level audit record.
    ///
    /// `riders` are the destination's: `x-amz-acl` sets the ACL of the *new* object, and
    /// `x-amz-tagging` under `x-amz-tagging-directive: REPLACE` sets its tags. They are
    /// screened and authorized exactly as on a plain write, and the destination bucket
    ...
    /// rather than a caller-side check so that every refusal is a `Deny` verdict here,
    /// before anything is written. A caller-side refusal lands after this function has
    /// audited `Outcome::Allowed` and minted a proof, and a record that contradicts what
    /// the client was told is worse than no record.
...
impl S3Access for GatewayAccess {
    async fn check(&self, cx: &mut S3AccessContext<'_>) -> S3Result<()> {
        // Every branch below emits an audit record as well as a log line: this is the
        // whole deny-by-default surface, and without evidence "was this gateway probed?"
        // is unanswerable. The access-key id is a semi-public identifier and is recorded;
        // the secret never is.
        //
        // The sink rate-limits these (`emit_gate_denial`): an unauthenticated caller can
        // produce them at line rate, and a queue full of scanner noise would evict the
        // decision records of real access. Suppression is counted, never silent.
    ...
    ///
    /// A `PutObject` carrying `x-amz-acl: public-read` is a visibility change, not just a
    /// write: reading only `bucket` and `key` would let the object become world-readable
    ...
    /// Browser form upload: the key is in the parsed multipart body, not in the path, so
    /// only a hook that reads the *parsed* request can authorize it.
    ///
    /// s3s aggregates the whole file into memory during route resolution, ahead of `check`
    /// and of everything here, so an unauthenticated caller can make this process allocate
    /// up to `limits.post_object_max_file_size` before any authorization runs — which is
    /// why that default is 64 MiB. Nothing in this hook can move that bound.
    ///
    /// Its `acl` and `tagging` arrive as **form fields** covered by the signed POST policy
    /// document, so stripping them after signature verification would forward a body that
    /// no longer matches what the caller signed.
    ...
    ///
    /// Left unread, `x-amz-bypass-governance-retention` would make a `delete_objects`
    /// grant silently include "and may destroy retained records" on a bucket with object
    /// lock in governance mode. See [`Self::screen_riders`].
    ...
    async fn delete_objects(&self, req: &mut S3Request<DeleteObjectsInput>) -> S3Result<()> {
        // Multi-delete keys live in the XML body — authorize EACH key, then filter the
        // forwarded request to only the allowed keys. The cap is read once: two loads
        // could straddle a config apply and report a cap the request was not measured
        // against.
        ...
        // too many, and an audit record is not the place to copy an unbounded list.
        ...
        // The bypass header covers the WHOLE batch, so it is screened once, before any key
        // is decided, and refuses the whole request. A partially-honored WORM bypass is
        // not a thing this gateway should be able to produce.
    ...
    async fn copy_object(&self, req: &mut S3Request<CopyObjectInput>) -> S3Result<()> {
        ...
        // which the gateway does not read (`object_tags`, ADR-003's gated on-demand fetch,
        // still unwired). So a REPLACE directive is a tag write and is authorized as one,
        // including a REPLACE with no header, which installs the empty set. A header sent
        // under COPY is authorized rather than ignored. What rides through unauthorized is
        // the inherited set under COPY, which is on the op's blind-spot list.
        let replace = req
            .input
            .tagging_directive
            .as_ref()
    ...
    /// Enumerate the caller's buckets — the only hook here that does not answer a refusal
    /// with a 403.
    ///
    /// "May not enumerate" and "may enumerate, holds no bucket grants" must be
    /// **indistinguishable**, or the status code answers "does this credential hold
    /// `list_buckets` in this tenant?" for anyone who asks. Both return `200` with an
    /// empty `<Buckets/>` through the same backend-free branch; only the audit record
    /// tells them apart. Deny-by-default is intact: with no obligation the visibility is
    /// [`BucketVisibility::Nothing`].
    ///
    ...
    // ── bucket existence (client compatibility) ─────────────────────────────────
    //
    // Deliberately absent: `create_bucket`, `delete_bucket`, `{get,put}_bucket_policy`
    // and `{get,put}_bucket_cors`. The gateway is data-plane only, and these are REMOVED
    // rather than stubbed — a stub returning `Ok(())` is the s3s fail-OPEN default wearing
    // a comment. With no hook and no dispatch arm, `OP_TABLE` marks each
    // `Coverage::Denied`, `check` refuses before deserialization, and even a bug in
    // `check` lands on s3s's `NotImplemented`;
    // `tests/gate_invariants.rs::denied_ops_have_no_dispatch_arm` holds that.
    //
    // Bucket existence, policy, CORS and quota are properties of the managed resource and
    // are changed through the control plane. See `optable::NON_GATEWAY_VERBS`.
    ...
    /// `read_objects`, the same verb as `GetObject`.
    ///
    /// Reading an object's tags is strictly less than reading the object itself, so a
    /// separate `read_object_tags` verb buys no containment and costs a grant an
    /// administrator has to know to give.
    ///
    ...
    /// object's current one — because a principal that can set the key its own grant
    /// conditions read can elevate itself. Deciding against the current set would
    /// authorize the state the object is leaving, not the one it is entering.
    ///
    /// Necessary but not sufficient, so the reserved-key guard runs here too and runs
    /// *first*: see [`tagging`] for why an absent `reserved_tag_keys` list refuses every
    /// tag write rather than reserving nothing.
    ...
    /// Reading the current set first would be ADR-003's gated `object_tags` fetch, which
    /// is deliberately not wired.
    ///
    /// It is still a **tag write**, so it is refused while tagging is inert: removing a
    /// tag changes the ABAC facts about an object just as setting one does, and a
    /// deployment whose control plane has not said which keys a policy may depend on
    /// cannot tell which removals matter.
...
///
/// AWS clients send a `list`-shaped header as one comma-separated header
/// (`x-amz-object-attributes: ETag,ObjectSize`), but `s3s`'s `parse_list_header` iterates
/// `headers.get_all(name)` and never splits on the comma, so the forward re-encodes
/// `["ETag,ObjectSize"]` through the AWS SDK — which quotes any element containing a
/// comma — and the backend answers `400 InvalidArgument`.
///
/// A canonicalization, not an authorization decision, and it happens in the hook so the
/// value the PDP saw, the value on the audit record and the value forwarded are one.
...
/// (`proxy::fan_out_list_v2`, ADR-004). Everything else must refuse a multi-prefix grant
/// as part of the *verdict*, so it is audited as the denial it is.
...
fn classify_list(decision: &Decision, max_fanout: usize, fanout: Fanout) -> ListVerdict {
    ...
    if !allowed.is_empty() {
        // Refuse here, not at the caller: a caller-side refusal lands *after*
        // `enforce_list` has audited and minted a proof, making the decision log disagree
...
/// [`BucketListing::Withhold`], including an **absent** obligation — the asymmetry
/// documented on [`crate::authz::Obligations::visible_buckets`], which makes a policy that
/// said nothing about bucket visibility safe rather than catastrophic.
...
fn list_verdict_single(verdict: ListVerdict, prefix: &mut Option<String>) -> S3Result<()> {
    match verdict {
        ...
        // `classify_list` returns `Deny` instead. Kept as a compile-checked backstop — if
        // an edit makes it reachable, the request fails rather than being forwarded
        // unbounded, and says the invariant broke rather than blaming the client.
...
///
/// Two refusals, both about the PDP and the backend seeing the same thing:
///
/// - **the cap** (`limits.max_tag_count`; AWS's own limit is 10);
/// - **duplicate keys.** A `TagSet` is a list, so folding it into a map silently keeps
///   one of them while the backend keeps whichever *it* prefers — a parser differential
///   in which the policy authorizes a tag set the object never receives. A tag with no
...
mod tests {
    ...
    fn the_gate_admits_the_blind_spot_ops() {
        ...
        // The form upload's key lives in the parsed multipart body, so only a typed hook
        // can authorize it, and its s3s trait default *forwards* rather than refusing.
        assert!(optable::gate_op("PostObject").is_ok());
        // ListBuckets is the only op whose response the gateway rewrites, so its hook is
        // what installs the visibility obligation the dispatcher applies.
    ...
    fn list_single_prefix_narrows() {
        ...
        // A narrowing needs no fan-out dispatch, so it must survive on an op that cannot
        // fan out, or every v1 list by a prefix-scoped subject would be a denial.
        assert!(
    ...
    fn a_fanout_an_operation_cannot_execute_is_a_deny_verdict_not_a_caller_override() {
        // `enforce_list` audits from the verdict, so a refusal that lives at the *caller*
        // is one the audit record never sees: it would log `Outcome::Allowed`, mint a
        // proof, and hand the client a 403. Both refusal sites — a v1/multipart list, and
        // a v2 list carrying a delimiter — must show up here as `Deny`, carrying the
        // message the client is about to be given.
        let d = allow_with(Obligations {
    ...
    fn an_allow_with_no_visibility_obligation_shows_nothing() {
        ...
        // visibility must yield an empty listing, never an unfiltered one.
        assert_eq!(
    ...
    fn a_denied_enumeration_and_an_empty_grant_set_are_the_same_answer() {
        // The caller must not be able to tell "you may not enumerate" from "you may, and
        // you hold nothing": both are Nothing, so both produce the same empty listing
        // through the same branch at the same cost. Only the audit record separates them.

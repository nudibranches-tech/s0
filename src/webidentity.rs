//! `Action=AssumeRoleWithWebIdentity` — the AWS STS wire protocol, so any S3 SDK can
//! obtain a gateway credential with stock configuration.
//!
//! The tenant travels in the **`RoleArn` — a request parameter, not a token claim**, since
//! an IdP cannot carry a per-tenant claim. The organization comes from s0's own tenant→org
//! binding ([`crate::proxy::BackendRegistry`]); the role name itself confers no authority.
//!
//! The listener is unauthenticated by design — a valid token *is* the credential, as at
//! `sts.amazonaws.com` — so it must never share a socket with the internal, admin or S3
//! listeners; [`crate::config::GatewayConfig::validate`] refuses a config where it does.
...
/// The identity provider names a service account `service-account-<clientId>` in
/// `preferred_username`. This prefix is the only reliable in-token marker of the class.
...
/// retries some, never retries others, and surfaces `ValidationError` as a configuration
/// mistake.
...
pub enum StsRefusal {
    ...
    ///
    /// The message is uniform and says nothing about *which* half failed — this endpoint
    /// is internet-reachable and the detail is a tenant-existence oracle. It is logged.
    AccessDenied,
...
impl StsRefusal {
    ...
    /// `Sender` for anything the caller can fix, `Receiver` for anything it cannot — the
    /// AWS convention, and the signal a client uses to decide whether to retry.
...
impl AssumeRoleWithWebIdentityRequest {
    ...
    pub fn from_params(params: &Params) -> Result<Self, StsRefusal> {
        ...
        // satisfy, so it is a malformed request rather than one with a ceiling.
...
/// assumed-role ARN. It is escaped there too ([`xml_escape`]); the charset check means it
/// can never carry markup in the first place.
...
///
/// Hand-rolled rather than pulled from a crate: it sits on the unauthenticated edge of a
/// credential-minting endpoint, where a new dependency is the costlier option.
#[derive(Debug, Default, Clone, PartialEq, Eq)]
pub struct Params(Vec<(String, String)>);

impl Params {
    /// Decode a form body or a query string. Never fails: an undecodable escape is left
    /// as written, since every value this surface reads is validated on its own terms and
    /// refusing a whole document over one stray `%` turns a client quirk into an outage.
    ...
    /// The **first** value for `name`, as the AWS query protocol specifies. First, not
    /// last: a caller sending `WebIdentityToken` twice must not be able to make the value
    /// that is *verified* differ from the value a middlebox logged.
...
///
/// **Works on bytes throughout, never indexing the `str` by a computed offset.** Slicing
/// `&input[i + 1..i + 3]` panics when a `%` is followed by a multi-byte character (`%€`
/// suffices), which on this unauthenticated endpoint is a remotely triggerable panic.
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RoleArn {
    /// The ARN's *account* field, which here is the **Ceph tenant** — the same shape
    /// (`arn:aws:iam::{tenant}:role/{tenant}-sts-role`) an operator provisions RGW's own
    /// roles under, so a client repointed from RGW STS to s0 changes only its endpoint.
    pub tenant: String,
...
impl RoleArn {
    ...
    ///
    /// Lenient only about the partition. Two rules protect the tenant:
    ///
    /// * the **region field must be empty** — IAM is global, so a non-empty one means the
    ///   caller is not sending what it thinks it is, and the field it *has* filled may be
    ///   the one read as the tenant;
    /// * the resource must be `role/<non-empty>` and the account non-empty, so the
    ///   untenanted `arn:aws:iam:::role/x` is refused rather than selecting nothing.
...
/// Normalize a role name for comparison: non-alphanumerics become `-`, case is folded.
///
/// The expected name substitutes a *tenant* into a template, but tenants and role names
/// are sanitized differently on the way into the backend (a Ceph tenant may contain `_`,
/// an RGW IAM role name may not), so the two differ in separators even when everything is
/// correct. Folding is safe because the role name is not an authorization input: a
/// separator difference must not cost a tenant its credentials, and a typo still does not
...
pub struct WebIdentityPrincipal {
    ...
    /// **clientId**. Never prefixed — the rego composes `user:`/`sa:` itself.
    pub sub: String,
...
/// Decide user vs service account from the token's claims.
///
/// The classes are different key spaces in the bundle (`sa:<clientId>` vs `user:<sub>`),
/// so guessing wrong evaluates against no grants at all. A `preferred_username` beginning
/// `service-account-` marks a service account, whose clientId comes from `azp` (or
/// `client_id`) because that carries the registered spelling the bundle is keyed on,
/// while `preferred_username` may be lowercased; if `azp` disagrees with the username's
/// suffix the token is refused rather than reconciled. Everything else is a user keyed by
/// `sub`, and an empty subject is refused: `sa:`/`user:` are keys a grant could match.
...
///
/// The second, purely **additive** route by which a token can be found to be addressed to
/// this gateway — see [`WebIdentitySts::addressed_to_this_gateway`].
///
/// Implemented by [`crate::pdp::BundleStore`], the same revision-swapped store the PDP
/// decides against, so every call reads the bundle in force at that instant. A cache here
/// would be a second answer to "what does the control plane currently say", and a
/// revocation could then land in the PDP and not here.
pub trait TenantSubjects: Send + Sync {
    /// Is `sa:<client_id>` a **member subject** of `tenant` in the bundle in force?
    ///
    /// "Member subject" is the policy's own word: `s3.rego`'s membership rule is
    /// `member if is_object(data.tenants[input.tenant].user_attributes[subject_key])`,
    /// and this asks that question, of that map, under that key.
    fn knows_service_account(&self, tenant: &str, client_id: &str) -> bool;
}

/// The client id a token names in `azp`, falling back to `client_id`.
///
/// **The only claim the bundle route reads.** Never `aud` — a token may carry an audience
/// it does not speak for — and never `sub`, which is the other key space (`user:`): no
/// claim on a human's token may select an `sa:` key.
...
/// `VerifiedIdentity` requires a tenant and an organization claim, the very requirement
/// this surface removes — the tenant comes from the `RoleArn`, the organization from the
/// routing table.
...
pub trait WebIdentityVerifier: Send + Sync {
    ...
    /// gateway: that decision needs the tenant out of the `RoleArn`, which this trait
    /// never sees. [`WebIdentitySts::addressed_to_this_gateway`] makes it on the one path
    /// that can reach a credential, so no implementation here can skip it by omission.
    ...
    /// or `azp`/`client_id`. Nothing narrows this answer — the bundle route is only ever
    /// consulted after it says no.
    fn audience_is_accepted(&self, claims: &Value) -> bool;

    /// The configured list, for a refusal **log line**. Never for a response body: this
    /// endpoint is internet-reachable and the list names the OIDC clients it serves.
...
impl WebIdentityConfig {
    ...
    pub fn from_config(cfg: &StsMintConfig, session_ttl: Duration) -> Self {
        WebIdentityConfig {
            ...
            // nothing gets the same lifetime every other door hands out.
...
///
/// Holds the **same** [`StsAuthority`] the S3 front verifies with, so a credential minted
/// here is byte-compatible with one minted by any other door: same key ring, same
/// `HFST<kid>.<sid>` shape, same derived secret, same retirement semantics. A second
/// authority could mint credentials the data plane would not honour, and both sides would
...
pub struct WebIdentitySts {
    ...
    /// The bundle-driven half of the audience decision. `None` ⇒ the configured audience
    /// list is the only route. See [`Self::with_bundle_subjects`].
...
impl WebIdentitySts {
    ...
    /// Attach the policy bundle as a second, **additive** source of audience acceptance —
    /// see [`Self::addressed_to_this_gateway`] for the rule.
    ///
    /// A builder rather than a constructor parameter, so that not wiring one is a real,
    /// supported state: the configured audience list is then the only route.
    #[must_use]
    ...
    /// **Clamp, not refuse** — a deliberate deviation from AWS, which answers
    /// `ValidationError` above the role's `MaxSessionDuration`, and the same choice
    /// [`crate::internal::InternalApi::clamp_ttl`] makes.
    ///
    /// SDK web-identity providers schedule their refresh off the response's `Expiration`,
    /// so a clamped credential simply works, while a refusal is a hard failure at
    /// credential acquisition and the workload never starts. Clamping can only shorten a
    /// credential's life. Below AWS's 900 s floor is left alone rather than clamped up.
    ...
    /// **Is this token addressed to this gateway?** Two routes; the second runs only
    /// after the first has said no.
    ///
    ...
    ///    `audience`, so the list is never empty).
    /// 2. **The policy bundle** — `azp`/`client_id` `C` names a service account the
    ///    *current* bundle knows as a member subject
    ///    (`sa:C` ∈ `data.tenants[<tenant from the RoleArn>].user_attributes`). Route 1
    ///    alone cannot serve a tenant's own service accounts, whose tokens carry an
    ///    IdP-default `aud` and a clientId no rendered list names, without one operator
    ///    re-render per service account.
    ///
    /// Route 2 is additive and safe: acceptance is not authorization, so every request is
    /// still authorized against the bundle and a known subject with no grant is denied; it
    /// fails closed, since an empty or stale bundle knows *fewer* subjects; and it is
    /// scoped to the tenant the `RoleArn` names and cannot cross an organization, a bundle
    /// being published per organization (`user_attributes` is projected org-wide, so in
    /// practice it means "a service account of this organization"). `user_attributes` and
    /// not `s3_grants`, because it is the map the policy's own membership rule reads and a
    /// subject whose grants are not assigned yet must still be able to get a credential.
    ///
    /// It applies to this door only ([`crate::mint::StandardVerifier::verify`]'s strict
    /// `aud` check is untouched), reads only `azp`/`client_id` — never `aud`, never
    /// `sub` — and composes the `sa:` prefix itself, so no claim on a human's token can
    /// select a service account's key.
    fn addressed_to_this_gateway(&self, role: &RoleArn, claims: &Value) -> Result<(), StsRefusal> {
        ...
        // Two different operator problems, and only the log tells them apart: "this new
        // service account cannot get a credential" is a bundle question, "nothing from
        // this client works" an audience one. The RESPONSE stays uniform — see below.
        ...
        // One message for every refusal above: distinguishing them here would let an
        // unauthenticated caller tell "that tenant's bundle does not know you" from
        // "wrong audience", which is a tenant oracle. The accepted list is not echoed.
    ...
    ///
    /// No authority rests on the answer — the bundle decides that — so this is a
    /// *diagnostic* gate: without it `arn:aws:iam::acme:role/typo` mints a perfectly good
    /// `acme` session, and the ARN a client is told to use is not the ARN that works.
    ///
    /// Unconfigured ⇒ any non-empty role name is accepted, for deployments with no such
    /// naming convention. See [`role_name_key`] for why the comparison is loose.
    ...
    pub async fn assume_role_from_params(
        ...
        // A *wrong* `Version` is refused rather than ignored: the response document's
        // shape is versioned, and answering a client that asked for a different contract
        // is how a silent incompatibility ships.
    ...
    pub async fn assume_role(
        ...
        // The ARN is parsed first because it is a pure syntax check — a malformed one is
        // a client bug and deserves to say so. What must NOT happen before the token is
        // verified is the *routing* lookup below: that one is a tenant-existence oracle.
        ...
        let claims = self
            ...
            .await?;
        // The audience decision belongs here — after signature/issuer/expiry, before the
        // principal is resolved — so that an `InvalidIdentityToken` refusal never becomes
        // an `IDPRejectedClaim` or the other way round.
        ...
        let claims_for_session = SessionClaims {
            ...
            // Resolved from the token rather than hard-coded, so a service account
            // evaluates in the `sa:` key space.
            principal_type: principal.principal_type,
            // Deliberately empty. The policy reads groups from the bundle, never from the
            // session; a token's own role claims are an unvalidated IdP assertion about
            // authorization, and carrying them here would put a caller-influenced value
            // where an advisory audit field is read.
            groups: Vec::new(),
...
///
/// **`Expiration` is ISO-8601**, unlike the JSON mint's Unix integer: AWS SDKs parse this
/// field as an ISO-8601 date-time and a numeric value is a hard parse error, so getting it
/// wrong makes every SDK reject an otherwise perfect credential.
...
/// Render the error document, in the shape AWS STS uses (an `ErrorResponse` envelope, not
/// S3's bare `Error` root). An SDK's STS deserializer looks for this envelope; without it
/// every refusal, however precise its code, surfaces as an opaque "unknown error".
...
///
/// An out-of-range value degrades to the epoch rather than panicking: a credential
/// document is not a place to `unwrap`.
...
///
/// Applied to **every** interpolated value, including those whose charset is already
/// validated: a field that is safe only because of a check elsewhere becomes unsafe the
/// moment that check moves.
...
mod tests {
    ...
    /// Guards against a remote, unauthenticated panic: reading the two hex digits with
    /// `&input[i + 1..i + 3]` indexes a `str` at a non-boundary offset whenever a `%` is
    /// followed by a multi-byte character, so `RoleSessionName=%€` would kill the
    /// connection task. Every byte here is one an anonymous caller chooses, and the test
    /// returning at all is the guard.
    #[test]
    fn a_multibyte_character_after_a_percent_does_not_panic_the_decoder() {
        // 2-byte, 3-byte and 4-byte continuations, in the three positions where a
        // two-byte slice could straddle one.
        ...
        assert_eq!(p.get("Action"), Some("AssumeRoleWithWebIdentity"));
        // POSITIVE CONTROL: ordinary escapes still decode, so this does not pass by the
    ...
    fn the_tenant_comes_out_of_the_role_arn() {
        // The ARN shape an operator provisions in RGW.
    ...
    fn a_role_arn_that_names_no_tenant_is_refused() {
        ...
        // It selects nothing here, and must say so rather than resolve to an empty tenant.
        for bad in [
    ...
    /// The mismatch the normalization exists for is real: a Ceph tenant may carry `_`
    /// where the RGW IAM role name of the same tenant carries `-`, so the ARN an operator
    /// really provisions is not `{tenant}-sts-role` byte for byte. A strict comparison
    /// would 403 that exact ARN.
    #[test]
    fn the_role_name_the_operator_really_provisions_is_accepted() {
        let template = "{tenant}-sts-role";
        for (tenant, provisioned) in [
            // The no-sanitization case: the name is already a valid tenant.
            ("default", "default-sts-role"),
            // The common case: a tenant sanitized out of a hyphenated name.
    ...
    fn sa_token(client_id: &str) -> Value {
        // What an IdP issues for `grant_type=client_credentials`.
        serde_json::json!({
    ...
    fn a_keycloak_service_account_token_resolves_to_the_client_id() {
        // The bundle keys an SA's grants under `sa:<clientId>`; the token's `sub` is the
        // service-account *user* id, which appears in no bundle and finds no grants.
    ...
    fn the_registered_client_id_spelling_wins_over_keycloaks_lowercased_username() {
        // The IdP lowercases `preferred_username`; the bundle is keyed on the clientId as
        // registered. Taking the username's suffix would look up `sa:pipeline-runner`
    ...
    fn a_human_token_resolves_to_a_user_keyed_by_sub() {
        ...
        // `azp` alone must NOT make a human look like a service account — every user's
        // token carries one.
    ...
    //
    // These drive the REAL `assume_role` over a real `BackendRegistry` and `StsAuthority`.
    // TWO routable tenants is the load-bearing part of the setup: it makes the
    // cross-tenant test a statement about the bundle lookup rather than about the routing
    // table, which would refuse an unroutable tenant whatever this code did.
    ...
    ///
    /// Written out rather than reached with `expect_err` because [`AssumedRoleSession`]
    /// deliberately does not implement `Debug`: three of its fields are a live credential,
    /// and `{:?}` failing to compile is what keeps them out of panics and log lines.
    ...
    /// A tenant's own service account as an IdP issues it: `aud` is the IdP default and
    /// `azp` the clientId, so nothing in it names a configured audience.
    ...
    /// **The case the bundle route exists for**: a service account in no rendered
    /// audience list, whose `aud` is the IdP's default, which the configured list alone
    /// refuses `InvalidIdentityToken`.
    #[tokio::test]
    async fn a_tenant_service_account_the_bundle_knows_is_accepted_though_no_audience_names_it() {
        let sts = door(
            ...
            // A typical rendered list: the org storage client and two control-plane
            // clients. The SA is in none of them.
        ...
        // NEGATIVE CONTROL, on identical claims and an identical list: with no bundle
        // wired the token is refused, so the acceptance above came from the bundle.
        let unwired = door(
            tenant_sa_claims("test-s0-before"),
            &["acme-storage-sa", "hf-console", "control-plane-sa"],
            None,
    ...
    /// **Additivity: nothing the configured list accepts may be refused.**
    ///
    /// Every shape that list accepts — `aud` as a string, `aud` as an array, `azp`,
    /// `client_id`, a human's token — is driven through the door twice: with no bundle,
    /// and with a bundle that knows nothing at all. A bundle route that could narrow
    /// route 1 fails here.
    #[tokio::test]
    async fn every_token_the_configured_list_accepts_today_is_still_accepted() {
        let accepted_today = [
            ...
            serde_json::json!({ "aud": "account", "client_id": "control-plane-sa",
                                "sub": "u-4" }),
            // A service account whose clientId the operator DID configure as an
            // audience: route 1, and it must not start depending on the bundle.
    ...
    /// Acceptance is decided **before** the role name and the routing table: a token the
    /// bundle route accepts still gets the uniform `AccessDenied` for a wrong role name,
    /// and a token neither route accepts answers the token error, not the tenant one.
    #[tokio::test]
    ...
    /// **The detail that decides whether any SDK can use this at all.** Every AWS SDK
    /// parses `Expiration` as ISO-8601 and treats a bare number as a malformed response,
    /// so a numeric value rejects a credential that is otherwise perfect.
    #[test]

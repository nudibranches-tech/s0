//! Proven by execution end to end: a session minted through the control-plane-mediated
//! endpoint is a credential the S3 data plane accepts, and the identity the control plane
//! asserted is the identity the policy decision is made against. Other tests of this path
//! resolve the credential themselves, which leaves the load-bearing question open — would
//! a real S3 client signing SigV4 with it get through? So this file boots the real
//! `internal::serve_on` mint and the real `server::serve_with_shutdown` S3 front on two
//! sockets sharing one `StsAuthority`, mints over HTTP, drives a **real SigV4-signed**
//! `GetObject`, and reads the `OpaInput` the enforce path handed the PDP out of the
//! capture sink. Both principal classes are driven against grants keyed to *different*
//! prefixes, so nothing here can pass on a constant.
...
/// The shared secret, as the operator renders it into `internal.shared_secret`.
const SHARED_SECRET: &str = "PLATFORM-SHARED-SECRET-e2e";

/// The tenant and organization `tests/common` routes. The caller asserts both, and s0
...
///
/// Two disjoint grants rather than one shared grant, deliberately: it makes "the decision
/// was made against the subject that was asserted" a *checkable* claim rather than
/// something inferred from the input document.
...
/// plane and `internal::serve_on` for the mint — over two real loopback sockets, with one
/// `StsAuthority` shared between them.
async fn harness(tag: &str) -> Harness {
    ...
    // The internal API, on its own socket, over the SAME identity/STS the front verifies
    // with — the exact expressions `main.rs` passes. A second authority here would make
    // the whole file measure a credential nothing else in the process could honour.
...
/// A credential as the caller receives it.
struct Session {
...
/// A control plane's session request document, field for field.
...
/// **A service-account session minted through the internal API is honoured by SigV4 and
/// evaluated as a service account** — the case s0's own OIDC mint cannot produce, since it
/// hard-codes `PrincipalType::User`.
#[tokio::test]
async fn a_console_minted_service_account_session_is_accepted_by_sigv4_and_carries_its_identity() {
    ...
    // request below would be answered by the static store and the token never consulted.
    assert!(
    ...
    // 1. SIGNATURE + IDENTITY. `sa/q1.csv` is inside the service account's grant, so the
    //    request passes `check` and the policy and reaches the forward — where the
    //    fixture's backend is not listening. Anything refused at the gate answers 403.
    ...
    //    `GatewayAccess::decide` — so this is what the PDP was handed, not a reconstruction.
...
/// The negative control for the SigV4 half: without it, "not 403" above would be satisfied
/// by a gateway that accepted everything. Three separable ways to hold a minted session
/// wrongly, each of which must fail — wrong secret (signature), no session token
/// (identity), and another session's token (binding).
#[tokio::test]

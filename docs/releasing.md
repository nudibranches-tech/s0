...
tagged commit, builds and publishes `90o7448p.c1.gra9.container-registry.ovh.net/s0/s0:0.3.3`, and attaches
the binary tarballs and the image digest to a GitHub Release. It **refuses to overwrite** a version
...
| | | |
|---|---|---|
| **Registry** | `90o7448p.c1.gra9.container-registry.ovh.net/s0/s0` | The organisation's own registry. CI authenticates with the `REGISTRY_USERNAME` / `REGISTRY_TOKEN` repository secrets; that token is a standing key to exactly the artifact this care is about, so it is scoped to push on `s0/s0` alone. The registry is private — give the pulling cluster a pull secret. An air-gapped consumer copies the tag onward (`skopeo copy` / `crane copy`): a byte-for-byte move, never a rebuild. |
